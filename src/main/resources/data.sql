...
3. Baadhi ya mifumo ya jua hutumia app maalum ya simu badala ya IP ya moja kwa moja - angalia app ya brand husika.',
  ...
  (25, 10, 'Smart TV Haifikiki', '', 'Haijibu kwenye mtandao',
   '1. Angalia kama TV ina umeme na iko imewashwa (si "standby" tu).
2. Thibitisha TV imeunganishwa kwenye Wi-Fi sahihi (angalia mipangilio ya mtandao ya TV yenyewe).
3. Jaribu kuzima na kuwasha upya TV (zima kwa umeme kabisa kwa sekunde 30).
4. Thibitisha IP address ya TV haijabadilika kwenye router.',
   'easy', 0, CURRENT_TIMESTAMP, 'TV_UNREACHABLE'),

  -- Simu Imefungwa (PIN/Pattern)
  (26, 1, 'Simu Imefungwa (PIN/Pattern Imesahaulika)', '', 'Mteja amesahau PIN au pattern ya simu',
   '1. NJIA SALAMA (haifuti data): Kama mteja anakumbuka akaunti yake ya Google, tumia "Find My Device" (google.com/android/find) kufungua kwa mbali.
2. Kwa Samsung: tumia "Find My Mobile" (findmymobile.samsung.com) - "Unlock my screen".
3. NJIA YA MWISHO (INAFUTA DATA YOTE): Factory Reset kupitia Recovery Mode - zima simu, kisha bonyeza mchanganyiko wa vitufe (Power+Volume, hutofautiana kulingana na brand), chagua "Wipe data/factory reset".
4. MUHIMU: Thibitisha umiliki wa simu (risiti ya ununuzi, IMEI, kitambulisho) KABLA ya kufanya factory reset - hii inazuia kufungua simu zilizoibiwa.',
   'medium', 0, CURRENT_TIMESTAMP, 'SCREEN_LOCKED');

-- Weka namba za IDENTITY zinazofuata sahihi baada ya kuingiza ID maalum
ALTER TABLE device_category ALTER COLUMN id RESTART WITH 11;
ALTER TABLE repair_guide ALTER COLUMN id RESTART WITH 27;
